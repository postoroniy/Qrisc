...
import Qrisc_pack::*;
    ...
    localparam int DMEM_SIZE = 10;
    localparam int DMEM_ADR_LIMIT = 10;
...
`include "axi_tb_connect.svh"
    ...
    typedef enum {reversed_unsorted, randomly_unsorted, already_sorted} data_kind_t;
	    ...
	    task validate_sorted_data;
	      integer i;
	      begin
	        for (i = 0; i < 9; i++) begin
	          if (Dram.sram[i] > Dram.sram[i + 1]) begin
	            $display("FAIL: sort order dmem[%0d]=%0d dmem[%0d]=%0d",
	                     i, Dram.sram[i], i + 1, Dram.sram[i + 1]);
	            $fatal(1);
	          end
	        end
	      end
