...
\begin{frame}
  \titlepage
  \begin{itemize}
    \item 5-stage RISC: IF, ID, EX, MEM, WB.
...
\begin{frame}{Instruction Decode Stage}
  \begin{itemize}
    ...
    \item Reads values from the standalone register file.
    \item Applies EX, MEM, and WB forwarding before driving the EX stage.
  \end{itemize}
\end{frame}
...
\begin{frame}{Execution Stage}
  \begin{itemize}
    ...
    \item Forwards ALU results and SRC2 increments to the bypass network.
    \item Provides one of the writeback sources.
  \end{itemize}
\end{frame}
...
\begin{frame}{Writeback Stage}
  \begin{itemize}
    \item Owns the architectural register-file commit path.
    \item Registers EX-stage and MEM-stage writeback sources.
    \item Writes EX/DST, EX/SRC2 increment, and MEM/DST results.
    \item Keeps deterministic priority when two sources target the same register.
  \end{itemize}
\end{frame}

\begin{frame}{Forwarding}
  \begin{itemize}
    \item ID can obtain register values from EX, MEM, WB, or the register file.
    \item MEM forwarding has priority over EX forwarding, matching the original SV behavior.
    \item WB forwarding covers values committed one stage after EX or MEM.
  \end{itemize}
\end{frame}
